...
zip_code = read_csv("./Data/Zip Codes.csv") |> 
  janitor::clean_names() |> 
  select(zip_code,everything(),-file_date)
```
...
zori = read.csv("./Data/Zip_zori_uc_sfrcondomfr_sm_month_NYC.csv") |> 
  ...
  select(zip_code,everything(),-region_type,-metro,-county_name,-city,-state,-state_name,-region_id)
...
| county_fips | 0 | 1.00 | 36055.58 | 24.58 | 36005.0 | 36047.00 | 36061.00 | 36081.00 | 36085.00 | ▃▁▆▇▇ |
...
There are 17516 observations with different date and rent index. The
data set included total of 149 zip codes as separate regions.
...
The zip codes are not included in ZORI data with some possible
situations that

- few or no rental listings in Zillow’s database
...
zori_compare <- zori_zip %>%
  ...
  pivot_wider(names_from = date, values_from = zori, names_prefix = "zori_") %>%
  mutate(price_drop = zori_2020_01_31 - zori_2021_01_31) %>%
  arrange(desc(price_drop)) %>%
  slice(1:10) %>%
  select(zip_code, county, neighborhood, zori_2020_01_31, zori_2021_01_31, price_drop)
```
